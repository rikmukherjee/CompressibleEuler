# === Second Code: Compare_EF_Vel (Corrected for alignment) ===
import numpy as np
...
ti= 1000
t = ti * dtt
vel = np.load(folder+'/uPerturb-t-%d.npy'%ti)
...
tStart = 1000 * dtt
plt.text(0.05, 0.97, r'$\textbf{(c)}$', transform=plt.gca().transAxes,verticalalignment='top', horizontalalignment='center')
...
# plt.ylim([min(vel)-0.000035, 1.5*max(np.abs(vel))])  # match with first
plt.ticklabel_format(axis='y', style='sci', scilimits=(-2, 2))
# plt.xlim([-np.pi,np.pi])  # match with first
# plt.ticklabel_format(axis='x', style='sci', scilimits=(-2, 2))  # optional
plt.xlabel(r'$x$')
plt.ylabel(r'$\Delta v(x,t)$')
plt.tight_layout()
plt.savefig('Plots/Compare_EF_Vel.png', dpi=300, bbox_inches=None)
plt.show()
